...
public:
	...
	//Flag width + ios::right + setprecision()
...
public:
	...
	void WriteListOfTrainersToFile()
	{
		int userChoice;
		string fileName;
		bool isWorking = true;
	...
	void ReadFromFileListOfTrainers(string fileName)
	{
		string buffer;
		bool isWorking = true;
		ifstream fin;
		...
		try
		{
			if (fin.is_open() == false || fin.bad() == true) throw runtime_error("File can not be open or critical error appears!");
		}
		catch (runtime_error exception)
		{
			cout << "Error: " << exception.what() << endl;
		...
		case 2:
			...
			while (isWorkingTrainingsMenu)
			{
				...
				case 3:
					...
					break;
				case 4:
					if (gym.IsEmpty() == false) gym.WriteListOfTrainersToFile();
					else cout << "\nDatabase is empty!\n";
					break;
				case 5:
					fileName = gym.EnterFileName();
					if (gym.FileIsEmpty(fileName) == false) gym.ReadFromFileListOfTrainers(fileName);
					else cout << "\nFile is empty!\n";
					break;
				case 0:
